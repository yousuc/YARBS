...
# yousc's Auto Rice Boostrapping Script (YARBS)
# License: GNU GPLv3

### OPTIONS AND VARIABLES ###

dotfilesrepo="https://github.com/yousuc/voidrice.git"
progsfile="https://raw.githubusercontent.com/yousuc/YARBS/master/progs.csv"
aurhelper="yay"
repobranch="master"
export TERM=ansi
...
welcomemsg() {
	whiptail --title "Welcome!" \
		--msgbox "Welcome to yousuc's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-yousuc" 10 60
...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. YARBS can install for a user already existing, but it will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\nYARBS will NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that YARBS will change $name's password to the one you just gave." 14 70
}
...
maininstall() {
	...
	whiptail --title "YARBS Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 9 70
	installpkg "$1"
}
...
gitmakeinstall() {
	...
	whiptail --title "YARBS Installation" \
...
makeuserjs(){
	...
	userjs="$pdir/user.js"
	ln -fs "/home/$name/.config/firefox/yarbs.js" "$overrides"
...
trap 'rm -f /etc/sudoers.d/yarbs-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL" >/etc/sudoers.d/yarbs-temp
...
echo "%wheel ALL=(ALL:ALL) ALL" >/etc/sudoers.d/00-yarbs-wheel-can-sudo
echo "%wheel ALL=(ALL:ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syyu,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/loadkeys,/usr/bin/pacman -Syyuw --noconfirm,/usr/bin/pacman -S -u -y --config /etc/pacman.conf --,/usr/bin/pacman -S -y -u --config /etc/pacman.conf --" >/etc/sudoers.d/01-yarbs-cmds-without-password
echo "Defaults editor=/usr/bin/nvim" >/etc/sudoers.d/02-yarbs-visudo-editor
